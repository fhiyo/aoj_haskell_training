...
main :: IO ()
main = getContents >>= putStrLn . (\(f, s) -> f ++ " " ++ s) . extractFreqAndLongestWord . map (length &&& head) . makeFreqList [[]] . sort . words

extractFreqAndLongestWord :: [(Int, String)] -> (String, String)
extractFreqAndLongestWord list = (snd (minimumBy (flip compare) list), snd . head $ sortBy (flip . comparing $ length . snd) list)
